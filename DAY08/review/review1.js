...
const quiz2 = nintendo.filter((x) => x.genre.some((y) => y == "어드벤처") && x.price >= 60000);
console.log(quiz2);

// 3. 멀티 플레이어가 안되고, 장르가 어드벤처인 게임 리스트
const quiz3 = nintendo.filter((x) => x.multiplayer != true && x.genre.some((y) => y == "어드벤처"));
console.log(quiz3);

// some()과 every()의 중요성을 잘 알게 되었다..
// + 어떤 배열 요소에 특정값이 "있는지 없는지"를 확인할때,
// 확인하고 싶은 값이 "문자"라면, 
// 1. [some() 사용] ex) .filter((x) => x.내용.some((y) => y == "확인할 값")); 하는 방식도 있지만,

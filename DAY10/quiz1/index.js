...
const profile_template = ((item) => 
  `
    <div class="profilecard">
      <img src="${item.image}" alt="아무 의미없는 아이콘">
      <div class="info">
        <span class="name">${item.firstName + " " + item.maidenName + " " + item.lastName}</span>
        <span class="birth">${(item.birthDate).replaceAll("-", "/")}</span>
        <span class="department">${item.company.department}</span>
        <span class="email"><a href="https://${item.email}">${item.email}</a></span>
      </div>
    </div>
  `
);
...
data.users.forEach((x) => container.insertAdjacentHTML("beforeend", profile_template(x)));